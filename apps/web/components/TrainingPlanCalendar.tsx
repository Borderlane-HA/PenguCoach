...
function garminErrorText(value:string,de:boolean){
  if(value.startsWith("GARMIN_EXERCISE_NOT_FOUND:")){const name=value.split(":").slice(1).join(":");return de?`Garmin-Übung nicht gefunden: ${name}`:`Garmin exercise not found: ${name}`}
  if(value==="SPORT_NOT_SUPPORTED_BY_GARMIN_WORKOUT_EXPORT")return de?"Diese Sportart kann aktuell nicht als Garmin-Workout exportiert werden.":"This sport is not currently supported for Garmin workout export.";
  if(value==="STRUCTURED_STEPS_MISSING")return de?"Für den Garmin-Export fehlen strukturierte Trainingsschritte.":"Structured workout steps are missing for Garmin export.";
  return value.replace(/^ValueError:\s*/,"");
}
