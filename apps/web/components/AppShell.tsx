...
export default function AppShell({children,title}:{children:React.ReactNode;title?:string}){
  ...
  const appIcon=appearance?.has_app_icon?`/api/v1/settings/app-icon?v=${appearance.app_icon_version??0}`:null;
  ...
  return <div className="app-frame">
    <aside className="side-nav" aria-label={bi(lang,"Hauptnavigation","Main navigation")}>
      <a className="side-brand" href="/today"><span className={`brand-mark ${appIcon?"custom":""}`}>{appIcon?<img src={appIcon} alt=""/>:"P"}</span><span><strong>PenguCoach</strong><small>Training & Health</small></span></a>
