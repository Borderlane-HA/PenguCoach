...
type IconName="today"|"health"|"activities"|"training"|"coach"|"garmin"|"privacy"|"ai"|"users";
...
export default function AppShell({children,title}:{children:React.ReactNode;title?:string}){
  ...
  useEffect(()=>{api<Me>("/auth/me").then(v=>{if(v.safety_required)location.replace("/safety");else setMe(v)}).catch(()=>location.replace("/"))},[]);
  ...
  if(!me)return <main className="auth-loading"><div className="loading-mark"><span>P</span></div><p>{bi(lang,"PenguCoach wird geladen…","Loading PenguCoach…")}</p></main>;

  return <div className="app-frame">
    <aside className="side-nav" aria-label={bi(lang,"Hauptnavigation","Main navigation")}>
      <a className="side-brand" href="/today"><span className="brand-mark">P</span><span><strong>PenguCoach</strong><small>Training & Health</small></span></a>
      <div className="side-section">
        <div className="side-label">{bi(lang,"Übersicht","Overview")}</div>
        {nav.map(x=><a className={`side-link ${active(x.href)?"active":""}`} href={x.href} key={x.href}><Icon name={x.icon}/><span>{x.label}</span>{active(x.href)&&<i/>}</a>)}
      </div>
      <div className="side-section">
        <div className="side-label">{bi(lang,"Verbindungen & Einstellungen","Connections & settings")}</div>
        {settings.map(x=><a className={`side-link ${active(x.href)?"active":""}`} href={x.href} key={x.href}><Icon name={x.icon}/><span>{x.label}</span>{active(x.href)&&<i/>}</a>)}
      </div>
      {me.role==="admin"&&<div className="side-section">
        <div className="side-label">Admin</div>
        {admin.map(x=><a className={`side-link ${active(x.href)?"active":""}`} href={x.href} key={x.href}><Icon name={x.icon}/><span>{x.label}</span>{active(x.href)&&<i/>}</a>)}
      </div>}
      <div className="side-user"><span className="side-avatar">{initials}</span><span><strong>{me.username}</strong><small>{me.role}</small></span></div>
    </aside>

    <section className="app-main">
      <header className="topbar">
        <div className="top-context"><span className="top-kicker">PenguCoach</span><strong>{title??current?.label??bi(lang,"Übersicht","Overview")}</strong></div>
        <div className="top-actions">
          <span className="health-live"><i/>{bi(lang,"System aktiv","System online")}</span>
          <button className="language-switch" onClick={()=>setLang(lang==="de"?"en":"de")} aria-label={bi(lang,"Sprache wechseln","Change language")}>{lang==="de"?"DE":"EN"}<span>⌄</span></button>
          <div className="top-avatar" title={me.username}>{initials}</div>
        </div>
      </header>
      <main className="stack page">{title&&<div className="page-title"><h1>{title}</h1></div>}{children}</main>
    </section>

    <nav className="mobile-dock" aria-label={bi(lang,"Mobile Navigation","Mobile navigation")}>
      {nav.map(x=><a className={active(x.href)?"active":""} href={x.href} key={x.href}><Icon name={x.icon}/><span>{x.label}</span></a>)}
    </nav>
  </div>;
}
