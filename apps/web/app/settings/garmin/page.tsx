...
type Status={connected:boolean;status:string;display_name?:string;last_successful_sync_at?:string;next_sync_at?:string;cooldown_until?:string;last_error_code?:string;activity_count?:number;activity_catalog?:Record<string,any>|null;settings?:GarminSettings};
...
export default function GarminSettingsPage(){
  ...
  async function startActivityCatalog(){
    if(historyRunning||syncing)return;
    setHistoryOpen(true);setHistoryRunning(true);setHistoryProgress({phase:"activities",state:"queued",matched:0,pages:0,message:"Activity catalogue queued"});
    try{
      const r=await api<any>("/garmin/import/activities",{method:"POST"});
      localStorage.setItem(HISTORY_JOB_KEY,r.task_id);setHistoryJobId(r.task_id);
      await pollHistory(r.task_id);
    }catch(e){
      setHistoryRunning(false);setHistoryProgress(null);
