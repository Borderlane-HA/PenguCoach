...
export default function Appearance(){
  ...
  return <AppShell title={bi(lang,"Darstellung","Appearance")}>
    ...
    <div className="card asset-card"><div className="asset-preview icon-preview">{a?.has_app_icon?<img src={`/api/v1/settings/app-icon?v=${a.app_icon_version??0}`} alt=""/>:<img src="/pengucoach-icon.svg" alt=""/>}</div><div><span className="eyebrow">BRANDING</span><h2>{bi(lang,"Eigenes App-Icon","Custom app icon")}</h2><p className="muted">{bi(lang,"Ersetzt das Standard-PenguCoach-Icon in der Seitenleiste. Am besten quadratisch mit transparentem Hintergrund.","Replaces the default PenguCoach icon in the sidebar. A square image with transparent background works best.")}</p><div className="form-actions"><label className="upload-button">{bi(lang,"Icon auswählen","Choose icon")}<input hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>upload("app-icon",e)}/></label>{a?.has_app_icon&&<button className="ghost" onClick={()=>remove("app-icon")}>{bi(lang,"Entfernen","Remove")}</button>}</div></div></div></section>
  </AppShell>
}
