...
export default function Training(){
  ...
  async function generate(e:FormEvent){
    ...
    try{
      const r=await api<AnyObj>("/coach/training-plan/jobs",{method:"POST",body:JSON.stringify({goal_type:goalType,goal_text:goalText,experience,weeks,days_per_week:days,session_minutes:minutes,equipment,constraints,context_days:contextDays,context_data:contextData,prompt:prompt||null,model_id:model,max_tokens:tokens,context_window_tokens:ctx,locale:lang})});
  ...
  return <AppShell>
    ...
    <form className="card training-modern" onSubmit={generate}>
      ...
      <p className="muted ai-budget-note">{bi(lang,`Die AI kennt ihr effektives hartes Antwortlimit und plant ${weeks*days} Einheiten kompakt innerhalb dieses Budgets. Bei Ollama wird die Ausgabe jetzt gestreamt; lange Generierungen sind nicht mehr an die frühere 300-Sekunden-Antwortwartezeit gebunden.`,`The AI knows its effective hard response limit and budgets ${weeks*days} sessions compactly within it. Ollama output is now streamed, so long generations are no longer bound to the former 300-second response wait.`)}</p>
