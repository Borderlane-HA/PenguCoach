...
export default function Health(){
  ...
  useEffect(()=>{
    const path=period==="all"?"/health/range?all=true":`/health/range?days=${Number(period)}`;
    api<any>(path).then(setData);
  },[period]);
  useEffect(()=>{api<Vo2Data>("/health/vo2-history").then(setVo2)},[]);
  ...
  return <AppShell>
    ...
    <section className="card vo2-history-card">
      <div className="section-heading vo2-heading"><div><span className="eyebrow">VO₂MAX · GARMIN</span><h2>{bi(lang,"Laufen & Radfahren · gesamter Verlauf","Running & cycling · complete history")}</h2><p>{bi(lang,"Sportartspezifische Garmin-VO₂max-Werte aus deinen Aktivitätszusammenfassungen – über alle importierten Daten.","Sport-specific Garmin VO₂ max values from activity summaries across all imported history.")}</p></div><div className="vo2-current"><div><small>{bi(lang,"Laufen","Running")}</small><strong>{vo2.latest?.running??"—"}</strong></div><div><small>{bi(lang,"Radfahren","Cycling")}</small><strong>{vo2.latest?.cycling??"—"}</strong></div><span>ml/kg/min</span></div></div>
